...
public class JwtInterceptor extends HandlerInterceptorAdapter {
  ...
  /**
   * This function is called after a SubletDispatcher and before a Controller to check whether 1) a
   * token exists or not 2) a token is expired or not 3) a token is invalidated or not ; due to
   * {@link UserJWTController#logout}
   *
   * @param request
   * @param response
   * @param handler
